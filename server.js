...
io.on("connection",s=>{
 s.on("admin:create",(_,cb)=>{let c;do{c=String(Math.floor(100000+Math.random()*900000))}while(rooms.has(c));rooms.set(c,def());s.join(c);cb?.({ok:true,code:c,room:publicRoom(rooms.get(c))})});
 s.on("admin:join",({code},cb)=>{const r=rooms.get(code);if(!r)return cb?.({ok:false});s.join(code);cb?.({ok:true,room:publicRoom(r)})});
 s.on("admin:save",({code,settings},cb)=>{const r=rooms.get(code);if(!r||r.status==="playing")return cb?.({ok:false});r.settings={...r.settings,...settings};emitRoom(code);cb?.({ok:true})});
 s.on("admin:start",({code},cb)=>{const r=rooms.get(code);if(!r||r.status!=="waiting")return cb?.({ok:false});r.status="countdown";emitRoom(code);io.to(code).emit("game:countdown",{seconds:3});setTimeout(()=>{if(!rooms.has(code))return;r.status="playing";r.startedAt=Date.now();r.endsAt=r.settings.finishMode==="time"?r.startedAt+r.settings.duration*60000:null;emitRoom(code);io.to(code).emit("game:start",publicRoom(r));},3000);cb?.({ok:true})});
 s.on("admin:end",({code})=>finish(code));
 s.on("admin:delete",({code},cb)=>{if(!rooms.has(code))return cb?.({ok:false});io.to(code).emit("room:deleted");rooms.delete(code);cb?.({ok:true})});
